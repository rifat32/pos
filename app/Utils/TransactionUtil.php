...
class TransactionUtil extends Util
{
     ...
     * @return boolean
     */
    public function createSellTransaction($business_id, $input, $invoice_total, $user_id,$service_center)
    {
        
           $date = new DateTime('now',new DateTimezone('Asia/Dhaka'));
           $new_date_format= $date->modify('-3 hour')->format('Y-m-d h:i:s');
        ...
        $transaction = Transaction::create([
            ...
            'sub_table' =>$input['sub_table'],
            'res_table_id' =>$input['table_id'],
            'status' => $input['status'],
            ...
            'transaction_date' => $new_date_format,
            'tax_id' => $input['tax_rate_id'],
            'discount_type' => $input['discount_type'],
            'discount_amount' =>isset($input['discount_amount']) ? $input['discount_amount'] : 0.00,
     ...
     */
    public function createOrUpdateSellLines($transaction,$service_center, $products,$contact_id, $location_id, $return_deleted = false, $status_before = null, $extra_line_parameters = [])
    {
        $lines_formatted = [];
        $modifiers_array = [];
        $edit_ids = [0];
        ...
        foreach ($products as $product) {
            ...
            if (!empty($product['transaction_sell_lines_id'])) {
                ...
                $dish_type = $this->get_DishType($product['product_id'])->dish_type;
                
                    if($dish_type == 3)
                    {
                        $service_center_m = NULL;
                    }
                    elseif($dish_type == 13)
                    {
                        $service_center_m = 1;
                        ...
                        $service_center_m = $service_center;
                    }
                ...
                $line = [
                    ...
                    'service_center_id' => $service_center_m,
    ...
    public function get_DishType($product_id){
    ...
    public function editSellLine($product, $location_id, $status_before)
    {
        ...
        if ($status_before != 'draft') {
            ...
            $this->adjustQuantity($location_id, $product['product_id'], $product['variation_id'], $difference,$sell_line->service_center_id);
            
            if($difference !=0){
            $this->adjustQuantitypurchaselineandservicecenter($sell_line->id,$sell_line->product_id,$difference);
            }
        }
   ...
   public function deleteSellLines($transaction_line_ids, $location_id)
    {
        if (!empty($transaction_line_ids)) {
            ...
            foreach ($sell_lines as $line) {
                 returnStockInStockHistory($line->id,$line->product_id);
                $this->adjustQuantity($location_id, $line->product_id, $line->variation_id, $line->quantity,$line->service_center_id);
            ...
            TransactionSellLine::whereIn('id', $transaction_line_ids)
                ->delete();
             if (!empty($delete)) {
                DB::table('sell_line_deletes')->insert($delete);
            }
        }
    }
     ...
     * @return boolean
     */
    private function adjustQuantity($location_id, $product_id, $variation_id, $increment_qty,$service_center=null)
    {
        ...
        if ($increment_qty != 0) 
        {
            ...
            {
                 $checkInDishList = DB::table('kitchen_used_raw_items')
                                ->leftjoin('kitchen_dish_list','kitchen_dish_list.id','=','kitchen_used_raw_items.dish_id')
                                ->where('kitchen_dish_list.product_id', $product_id)
                                ->get();

                            if(isset($checkInDishList[0]->used_unit_id)){
                            foreach($checkInDishList as $checkInDish){

                                    $stock_checker =DB::table('variation_location_details')
                                    ...
                                    if($checkInDish->used_unit_id != '0'){
                                        ...
                                        $unit = DB::table('units')->where('id',$checkInDish->used_unit_id)->first();

                                        if($unit->child_value){
                                            $qty = $checkInDish->used_quantity/$unit->child_value;
                                        }else{
                                            $qty = $checkInDish->used_quantity;
                                        }
                                       ...
                                       $qty_available_update=$stock_checker->qty_available + ($qty*$increment_qty);
                                        DB::table('variation_location_details')
                                        ->where('product_id',$checkInDish->raw_item_id)
                                        ->where('location_id',1)
                                        ->update(['qty_available'=>$qty_available_update]);
                                }
                            }
            }
            else
            ...
            $stock_check = DB::table('service_stock_management')
                          ->select(DB::raw("SUM(remain_stock) as remain_stock"),'service_stock_management.*')
                          ->where('product_id', $row->raw_product_id)->where('service_center_id',$row->service_center_id)
                          ->first();
             ...
             DB::Table('service_stock_management')
              ->where('id',$stock_check->id)
              ->update(['remain_stock'=> $stock_check->remain_stock + ($qty*$increment_qty)]);
            }
        }
    }
}
...
//when update same sell line quantity lot track for stock
public function adjustQuantitypurchaselineandservicecenter($sell_line_id,$product_id,$stock){
$product=\App\Product::find($product_id);

if($stock <0){

    $final_stock=abs($stock);

    if($product->dish_type==3){
        

    }else{
        $this->mapPurchaseSell2($sell_line_id,$product_id,$final_stock);
    }

}else{

    if($product->dish_type==3){
        $dishes=\App\Kitchendishlist::with('raws','raws.product','raws.unit')
              ->where('product_id',$product_id)
              ->first();
            foreach ($dishes->raws as $key => $raw) {

                    if($raw->used_unit_id != '0'){

                        if($raw->unit->child_value){

                            $qty = $raw->used_quantity/$raw->unit->child_value;
                        }else{
                            $qty = $raw->used_quantity;
                        }
...
return true;
     ...
     * @return boolean
     */
    public function createOrUpdatePaymentLines($transaction, $payments, $business_id = 1, $user_id = null)
    {
        ...
        foreach ($payments as $payment) {
            ...
            if (!empty($payment['payment_id'])) {
                ...
                if ($this->num_uf($payment['amount']) != 0) {
                    ...
                    if ($transaction->type == 'purchase') {
                        $prefix_type = 'purchase_payment';
                        \App\TransferPaymentBalance::create(['user_id'=>auth()->user()->id,'transaction_id'=>$transaction->id,'amount'=>$this->num_uf($payment['amount'])]);
                    }
                    $ref_count = $this->setAndGetReferenceCount($prefix_type, $business_id);
                    //Generate reference number
                    $payment_ref_no = $this->generateReferenceNumber($prefix_type, $ref_count, $business_id);
...
$totalInternalbalance = $total_internal_received->sum('balance') - $total_internal_expense->sum('balance');
    ...
    public function getTotalInternalBanking($business_id, $location_id, $start_date = null, $end_date = null)
    {
     ...
     public function mapPurchaseSell1($business,$transaction_lines,$old_quantity = 0){
            $line_id=array();
        foreach ($transaction_lines as $key => $line) {
            $line_id[]=$line->id;
            $qty_difference=$line->quantity - $old_quantity;
            $product_id=$line->product_id;
            $product=Product::find($product_id);
            if($product->dish_type==3){
            ...
            foreach ($dishes->raws as $key => $raw) {
                ...
                $line_row=DB::table('purchase_lines as PL')
                        ->where('product_id',$raw->raw_item_id)
                        ->whereRaw('(PL.quantity_sold + PL.quantity_adjusted + PL.quantity_returned) < PL.quantity')
                        ->select('PL.id as purchase_lines_id',
                            DB::raw('(PL.quantity - (PL.quantity_sold + PL.quantity_adjusted +PL.quantity_returned)) AS quantity_available'),
                            'PL.quantity_sold as quantity_sold',
                            'PL.quantity_adjusted as quantity_adjusted',
                            'PL.quantity_returned as quantity_returned')
                ...
                foreach ($line_row as $key => $row) {
                    $qty_allocated = 0;
                            $qty_selling = $qty *$qty_difference;
                            //Check if qty_available is more or equal
                            if ($qty_selling <= $row->quantity_available) {
                                $qty_allocated = $qty_selling;
                                $qty_selling = 0;
                            ...
                            $purchase_sell_map1[] = ['sell_line_id' => $line->id,
                                'purchase_line_id' => $row->purchase_lines_id,
                                'quantity' => $qty_allocated,
                                'created_at' => \Carbon::now(),
                                'updated_at' => \Carbon::now()
                ...
                if(!empty($purchase_sell_map1)){
                    TransactionSellLinesPurchaseLines::insert($purchase_sell_map1);
                }
            }

            }else{
                $service=$line->service_center_id;
                $rowItem =\App\Drink::with('raws','raws.unit')
                          ->where('drinks.product_id', $product_id)
                ...
                $line_rows=DB::table('service_center_stock_histories as SH')
                    ->where('SH.product_id',$row->raw_product_id)
                    ->where('SH.service_center_id',$service)
                    ->whereRaw('(SH.quantity_sold) < SH.quantity')
                    ->select('id','SH.quantity_sold',DB::raw('(SH.quantity - SH.quantity_sold) as quantity_available'))
                    ->orderBy('id','asc')
                    ->get();
                    foreach ($line_rows as $key => $line_row) {
                            $qty_allocated = 0;
                            $qty_selling = $qty *$qty_difference;
                            //Check if qty_available is more or equal
                            if ($qty_selling <= $line_row->quantity_available) {
                                $qty_allocated = $qty_selling;
                                $qty_selling = 0;
                            } else {
                                $qty_selling = $qty_selling - $line_row->quantity_available;
                                $qty_allocated = $row->quantity_available;
                            }

                            $purchase_sell_map2[] = ['sell_line_id' => $line->id,
                                'service_center_history_id' => $line_row->id,
                                'quantity' => $qty_allocated,
                                'created_at' => \Carbon::now(),
                                'updated_at' => \Carbon::now()
                            ];

                            \App\Service_center_stock_history::where('id', $line_row->id)
                                    ->update(['quantity_sold' => $line_row->quantity_sold + $qty_allocated]);
                            if ($qty_selling == 0) {
                                break;
                            }
                    }

                    if(!empty($purchase_sell_map2)){
                    TransactionSellLinesPurchaseLines::insert($purchase_sell_map2);
                    }
        ...
        TransactionSellLine::whereIn('id',$line_id)->update(['is_new'=>null]);
        
    }

// lot track method number 2

    public function mapPurchaseSell2($sell_line_id,$product_id,$stock,$old_quantity=0){

            $line=DB::table('transaction_sell_lines')->where('id',$sell_line_id)->select('service_center_id')->first();
            $qty_difference=$stock - $old_quantity;
            $product=Product::find($product_id);
            if($product->dish_type==3){
            ...
            foreach ($dishes->raws as $key => $raw) {
                ...
                foreach ($line_row as $key => $row) {
                    $qty_allocated = 0;
                            ...
                            $purchase_sell_map1[] = ['sell_line_id' => $sell_line_id,
                                'purchase_line_id' => $row->purchase_lines_id,
                                'quantity' => $qty_allocated,
                                'created_at' => \Carbon::now(),
                                'updated_at' => \Carbon::now()
            ...
            TransactionSellLine::where('id',$sell_line_id)->update(['is_new'=>null]);
